...
public sealed class CatalogService
{
	...
	private void ParseSkins(JsonDocument doc)
	{
		using (doc)
		{
			...
			foreach (var skin in doc.RootElement.EnumerateArray())
			{
				...
				var image = skin.TryGetProperty("image", out var img) && img.ValueKind == JsonValueKind.String ? img.GetString() ?? "" : "";

				if (!paints.TryGetValue(defIndex, out var list))
					paints[defIndex] = list = [];
				list.Add(new PaintDef(paint, paintName, minFloat, maxFloat, legacy, image));
	...
	private void ParseAgents(JsonDocument doc)
	{
		using (doc)
		{
			...
			foreach (var agent in doc.RootElement.EnumerateArray())
			{
				...
				if (team == "terrorists")
					t.Add(new AgentDef(model, name, CsTeam.Terrorist, faction, ReadString(agent, "image")));
				else
					ct.Add(new AgentDef(model, name, CsTeam.CounterTerrorist, faction, ReadString(agent, "image")));
			}
	...
	private void ParseMusic(JsonDocument doc)
	{
		using (doc)
		{
			...
			var kitImages = new Dictionary<int, string>();
			...
			foreach (var kit in doc.RootElement.EnumerateArray())
			{
				...
				kits[id] = name;
				kitImages[id] = ReadString(kit, "image");
			}

			MusicKits = kits.Select(k => new MusicDef(k.Key, k.Value, kitImages.GetValueOrDefault(k.Key, ""))).OrderBy(k => k.Id).ToList();
		}
	}
	...
	private void ParsePins(JsonDocument doc)
	{
		using (doc)
		{
			...
			foreach (var item in doc.RootElement.EnumerateArray())
			{
				...
				pins.Add(new PinDef(id, name, GetPinMenuGroup(name), ReadString(item, "image")));
			}
