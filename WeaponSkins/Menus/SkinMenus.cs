...
public sealed class SkinMenus
{
	...
	private Dictionary<int, List<MenuItem>> paintItems = [];
	private Dictionary<int, List<MenuItem>> glovePaintItems = [];
	...
	// Prepare first-use skin, knife, glove and sticker menus while the server
	// is still loading. Install only the completed dictionaries on the game thread.
	internal Action PrepareCatalogMenus(string defaultLabel)
	{
		var paintsByWeapon = new Dictionary<int, List<MenuItem>>();
		var paintsByGlove = new Dictionary<int, List<MenuItem>>();
		var gloves = Catalog.Gloves.Select(glove => glove.DefIndex).ToHashSet();

		foreach (var (def, paints) in Catalog.Paints)
		{
			if (gloves.Contains(def))
			{
				var items = new List<MenuItem>(paints.Count);
				foreach (var paint in paints)
				{
					var value = paint;
					items.Add(new MenuItem(paint.Name, (p, s) => SetGlovePaint(p, s, value)));
				}
				paintsByGlove[def] = items;
			}
			else
			{
				var items = new List<MenuItem>(paints.Count + 1)
				{
					new(defaultLabel, (p, s) => SetPaint(p, s, null))
				};
				foreach (var paint in paints)
				{
					var value = paint;
				...
				paintsByWeapon[def] = items;
			}
		}

		var stickers = plugin.Config.Stickers.Enabled ? Catalog.Stickers : [];
		var preparedStickers = stickers.Count > 0 ? BuildStickerItems(stickers) : null;
		var preparedBrowser = stickers.Count > 0 ? BuildStickerBrowser(stickers) : null;

		return () =>
		{
			paintItems = paintsByWeapon;
			glovePaintItems = paintsByGlove;
			lock (stickerCacheSync)
			{
				stickerCacheGeneration++;
				stickerItems = preparedStickers;
				stickerBrowser = preparedBrowser;
				stickerPrewarmTask = null;
			}
		};
	}
