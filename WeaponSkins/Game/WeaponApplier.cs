...
public sealed class WeaponApplier
{
	...
	private const int GlockDefIndex = 4;

	private static readonly float[] AmmoRestoreDelays = [0.06f, 0.3f, 0.6f];

	private const float RenderWearStep = 0.001f;
	private const int MaxRenderWearSteps = 21;
	...
	{
		if (!stickerWear.TryGetValue(slot, out var byWeapon) || !byWeapon.TryGetValue(defIndex, out var stored))
			return null;

		var offset = stored.Steps * (double)RenderWearStep;
		var nudged = stored.BaseWear + offset;

		if (nudged > 1d)
			nudged = stored.BaseWear - offset;

		return (float)Math.Clamp(nudged, 0d, 1d);
	}
	...
	private void BumpStickerWear(int slot, int defIndex, float wear)
	{
		...
		var steps = byWeapon.TryGetValue(defIndex, out var previous) ? previous.Steps + 1 : 1;
		if (steps > MaxRenderWearSteps)
			steps = 1;

		byWeapon[defIndex] = new RenderWear(wear, steps);
	}
	...
	public void RefreshOwned(CCSPlayerController player, int defIndex)
	{
		...
		RefreshAll(player, slotCommand, defIndex == GlockDefIndex);
	}
	...
	public void BumpAllRenderWear(CCSPlayerController player)
	{
		var loadout = cache.Get(player);
		if (loadout == null)
			return;

		foreach (var (defIndex, entry) in loadout.For(player.Team).Weapons)
		{
			if (entry is { Paint: > 0 })
				BumpStickerWear(player.Slot, defIndex, entry.Wear);
		}
	...
	public void RefreshAll(CCSPlayerController player, string? slotCommand = null, bool includeKnife = false)
	{
		var loadout = cache.Get(player);
		...
		foreach (var handle in weapons)
		{
			...
			if (slotCommand != null && weaponSlot != slotCommand && !(isKnife && includeKnife))
				continue;
	...
	private void RestoreActiveWeaponSlot(RefreshContext context, string? slotCommand)
	{
		...
		plugin.AddTimer(0.05f, () =>
		{
			...
			if (GetSlotCommand(active) == slotCommand)
				return;
	...
	private static bool PlayerHasGlock(CCSPlayerController player)
	{
		var weapons = player.PlayerPawn.Value?.WeaponServices?.MyWeapons;
		if (weapons == null)
			return false;
		...
		{
			var weapon = handle.Value;
			if (weapon != null && weapon.IsValid && weapon.AttributeManager.Item.ItemDefinitionIndex == GlockDefIndex)
				return true;
		}
